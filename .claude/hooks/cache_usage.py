...
def cache_hit_runs(marked: list, events: list, compactions: list) -> list[dict]:
    """Return one session total with the former per-series rows as details."""
    hits = [
    ...
    return [{
        ...
        "models": models,
        "details": cache_hit_details(marked, events, compactions),
    }]


def cache_hit_details(marked: list, events: list, compactions: list) -> list[dict]:
    """Group adjacent hits exactly as the pre-summary Usage history did."""
    breakers = []
