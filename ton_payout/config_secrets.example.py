...
# Мастер-контракт USDT (джеттон). Пусто = официальный Tether в mainnet.
# Для testnet укажите адрес тестового USDT явно.
USDT_MASTER_ADDRESS = ""

# Master-сид для выдачи РЕАЛЬНЫХ адресов пользователям (модель A). Из него
# детерминированно выводятся адреса по subwallet_id = id пользователя.
# ВНИМАНИЕ: кто владеет этим сидом — владеет ключами всех выданных адресов.
# Может совпадать с TON_MNEMONIC или быть отдельным. Пусто = выдача выключена.
TON_MASTER_MNEMONIC = ""
