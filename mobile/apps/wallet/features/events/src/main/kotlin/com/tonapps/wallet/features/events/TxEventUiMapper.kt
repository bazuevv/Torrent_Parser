...
class TxEventUiMapper(
    ...
    private fun isSpam(event: TxEvent, wallet: WalletEntity): Boolean {
        ...
        return if (localSpamState == SpamTransactionState.UNKNOWN) {
            ...
            localSpamState == SpamTransactionState.SPAM
        }
    }
     ...
     * их нельзя повторить обычным переводом.
     */
    private fun canRepeat(
        action: TxAction,
        isSpam: Boolean,
    ...
    fun toUiItem(event: TxEvent, wallet: WalletEntity): UiEvent.Item {
        ...
        val actions = event.actions.mapIndexed { index, action ->
            ...
            val title = if (isSpam) {
                ...
                action.title ?: context.getString(action.type.nameRes)
            }
            ...
            UiEvent.Item.Action(
                ...
                date = formatTransactionTime(event.timestamp.toLong(), context.locale),
                ...
                canRepeat = canRepeat(action, isSpam, state)
            )
        }
        ...
        return UiEvent.Item(
            ...
            filterIds = event.txFilterIds().toImmutableList(),
            spam = actions.any { it.spam },
            progress = event.inProgress
        )
    }

    private fun formatTransactionTime(epochMillis: Long, locale: Locale): String {
        if (epochMillis <= 0) {
            return ""
        }
        ...
        val time = DateTimeFormatter.ofPattern("HH:mm", locale).format(zdt)
        val day = DateTimeFormatter.ofPattern("d", locale).format(zdt)
        val isThisMonth = zdt.year == now.year && zdt.month == now.month
        return if (isThisMonth) {
            time
        } else {
            "$day $month $time"
        }
...
private fun TxAction.filterIdsForUi(): List<Int> {
    ...
    if (type == ActionType.Received || type == ActionType.NftReceived || type == ActionType.WithdrawStake ||
        type == ActionType.Refund || type == ActionType.Swap
    ) {
        filters.add(3)
    }
    return filters
...
private fun TxEvent.txFilterIds(): List<Int> {
    return actions.flatMap { it.filterIdsForUi() }.distinct()
}
